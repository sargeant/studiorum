...
from src.core.loaders.omnidexer import Omnidexer, IndexEntry
...
class TestOmnidexer:
    ...
    async def test_get_all_by_type(self, loaded_omnidexer):
        ...
        assert all(hasattr(spell, 'level') for spell in all_spells)  # Spell-specific check
        
        all_creatures = omnidexer.get_all_by_type(ContentType.CREATURE)
        assert len(all_creatures) >= 1
        assert all(hasattr(creature, 'strength') for creature in all_creatures)  # Creature-specific check
