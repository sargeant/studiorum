...
from src.core.indexer.tag_resolver import TagResolver, TagMatch


class TestTagMatch:
    """Tests for TagMatch class."""
    
    def test_tag_match_parsing(self):
        ...
        match = TagMatch.parse("spell", "fireball|PHB|magical fire", "{@spell fireball|PHB|magical fire}")
        assert match.name == "fireball"
        assert match.source == "PHB"
        assert match.display_text == "magical fire"
        
        # Tag with page reference
        match = TagMatch.parse("adventure", "chapter 1|CoS|Into the Mists|21", "{@adventure chapter 1|CoS|Into the Mists|21}")
...
class TestTagResolver:
    ...
    def test_tag_resolver_creation(self, loaded_omnidexer):
        """Test basic tag resolver creation."""
        resolver = TagResolver(loaded_omnidexer)
        assert resolver is not None
        assert len(resolver._tag_handlers) > 0
    
    def test_custom_handler_registration(self, loaded_omnidexer):
    ...
    async def test_filter_and_loader_tag_omission(self, tag_resolver):
        ...
        assert "This  and  should be hidden." in result  # Double spaces from removed content
    ...
    async def test_adventure_and_book_tags(self, tag_resolver):
        ...
        test_cases = [
            ...
            ("Refer to {@book Player's Handbook|PHB|123}.", "Player's Handbook, p. 123"),
            ("In {@adventure Chapter 1|CoS|Into the Mists|21}.", "Into the Mists (p. 21)"),
        ]
