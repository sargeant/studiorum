...
class TestContentSourceManager:
    ...
    def manager(self, mock_config):
        ...
        with patch('src.core.sources.manager.get_content_config', return_value=mock_config):
    ...
    async def test_ensure_all_sources_no_sources(self, manager, mock_config):
        ...
        with patch('src.core.sources.manager.logger') as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with("No enabled content sources configured")

    @pytest.mark.asyncio
    async def test_ensure_all_sources_git_not_available(self, manager, mock_config, github_source):
    ...
    @pytest.mark.asyncio
    async def test_ensure_all_sources_success(self, manager, mock_config, github_source, directory_source):
    ...
    @pytest.mark.asyncio
    async def test_ensure_all_sources_with_exceptions(self, manager, mock_config, github_source):
        """Test ensure_all_sources handling exceptions."""
        mock_config.get_enabled_sources.return_value = [github_source]
        manager.github_manager.is_git_available.return_value = True
        
        # Test that the method doesn't raise even if there are exceptions
        with patch.object(manager, '_ensure_github_source', side_effect=Exception("Test error")):
    ...
    async def test_ensure_github_source_failure(self, manager, github_source):
        ...
        manager.github_manager.ensure_repository = AsyncMock(side_effect=Exception("Network error"))
    ...
    async def test_ensure_directory_source_success(self, manager, directory_source):
        ...
        with patch('src.core.sources.manager.logger') as mock_logger:
            ...
            mock_logger.info.assert_called_with("Directory source 'test-directory' is ready")

    @pytest.mark.asyncio
    async def test_ensure_directory_source_path_not_exists(self, manager, directory_source):
        """Test directory source ensuring when path doesn't exist."""
        directory_source.path = Path("/nonexistent/path")
        
        with patch('src.core.sources.manager.logger') as mock_logger:
            with pytest.raises(FileNotFoundError, match="Directory source path not found"):
                await manager._ensure_directory_source(directory_source)
    ...
    @pytest.mark.asyncio
    async def test_ensure_directory_source_not_directory(self, manager, directory_source, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_build_content_index_force_rebuild(self, manager, mock_config, github_source):
        """Test build_content_index with force rebuild."""
        manager._index_built = True
        mock_config.get_enabled_sources.return_value = [github_source]
        
        mock_files = [Path("/test/file1.json"), Path("/test/file2.json")]
        with patch.object(manager, '_get_source_files', return_value=mock_files) as mock_get_files:
            with patch('src.core.sources.manager.logger') as mock_logger:
    ...
    @pytest.mark.asyncio
    async def test_build_content_index_with_exception(self, manager, mock_config, github_source):
        """Test build_content_index handling exceptions."""
        mock_config.get_enabled_sources.return_value = [github_source]
        
        with patch.object(manager, '_get_source_files', side_effect=Exception("Test error")):
            with patch('src.core.sources.manager.logger') as mock_logger:
                await manager.build_content_index()
    ...
    @pytest.mark.asyncio
    async def test_get_source_files_directory(self, manager, directory_source, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_get_source_files_directory_not_exists(self, manager, directory_source):
    ...
    @pytest.mark.asyncio
    async def test_get_source_files_directory_none_path(self, manager, directory_source):
    ...
    async def test_get_source_files_unsupported_type(self, manager):
        ...
        with patch('src.core.sources.manager.logger') as mock_logger:
            ...
            assert result == []
            mock_logger.warning.assert_called_with("Unsupported source type: UNSUPPORTED")
    ...
    def test_get_source_info_github(self, manager, mock_config, github_source):
        ...
        manager._content_index = {github_source.name: [Path("/file1.json"), Path("/file2.json")]}
    ...
    def test_get_source_info_github_no_git_info(self, manager, mock_config, github_source):
    ...
    @pytest.mark.asyncio
    async def test_update_source_github_success(self, manager, mock_config, github_source):
    ...
    @pytest.mark.asyncio
    async def test_update_source_directory_success(self, manager, mock_config, directory_source):
    ...
    async def test_update_source_failure(self, manager, mock_config, github_source):
        ...
        with patch.object(manager, '_ensure_github_source', side_effect=Exception("Update failed")):
            with patch('src.core.sources.manager.logger') as mock_logger:
                result = await manager.update_source(github_source.name)
    ...
    @pytest.mark.asyncio
    async def test_remove_source_data_github_success(self, manager, mock_config, github_source):
    ...
    @pytest.mark.asyncio
    async def test_remove_source_data_directory_success(self, manager, mock_config, directory_source):
    ...
    @pytest.mark.asyncio
    async def test_remove_source_data_exception(self, manager, mock_config, github_source):
        """Test remove_source_data with exception."""
        mock_config.get_source_by_name.return_value = github_source
        manager.github_manager.remove_repository.side_effect = Exception("Removal failed")
    ...
    def test_get_statistics_success(self, manager, mock_config, github_source, directory_source):
...
class TestContentSourceManagerIntegration:
    ...
    async def test_directory_source_integration(self, tmp_path):
        ...
        spell_file.write_text('{"spell": [{"name": "Fireball"}]}' + 'x' * 50)  # Make it large enough
        
        items_file = data_dir / "items.json"
        items_file.write_text('{"item": [{"name": "Sword"}]}' + 'y' * 50)  # Make it large enough
