...
class ValidationReport:
    ...
    def add_load_result(self, content_type: str, file_path: Path, 
                       items_loaded: int, load_time: float):
...
class TestFullDatasetValidation:
    ...
    async def test_full_creature_dataset_validation(self):
        ...
        for creature_file in creature_files:
            ...
            report.add_load_result("creatures", creature_file, len(creatures), load_time)
    ...
    async def test_omnidexer_full_dataset_load(self):
        ...
        assert total_items > 1000, f"Expected to load substantial dataset, got {total_items} items"
        assert len(validation_warnings) <= warning_threshold, (
            f"Too many validation warnings: {len(validation_warnings)} > {warning_threshold}"
        )
    ...
    async def test_concurrent_dataset_loading(self):
        ...
        async def load_subset():
            ...
            data_paths = source_manager.get_data_paths()
            spell_files = data_paths.get(ContentType.SPELL, [])[:3]  # Just first 3 files
    ...
    def test_validation_warning_categorization_full_dataset(self):
        ...
        for warning in validation_warnings:
            ...
            elif any(keyword in warning.lower() for keyword in ["parse", "parsing", "unable"]):
                categories["parsing_errors"].append(warning)
            elif any(keyword in warning.lower() for keyword in ["unknown", "unrecognized", "unexpected"]):
        ...
        for category, warnings in categories.items():
            ...
            if warnings:
                # Show sample
                sample = warnings[0][:80] + "..." if len(warnings[0]) > 80 else warnings[0]
                print(f"    Sample: {sample}")
                
        # Analysis assertions
        total_warnings = len(validation_warnings)
        unknown_ratio = len(categories["unknown_structure_errors"]) / total_warnings if total_warnings > 0 else 0
        
        assert unknown_ratio < 0.1, (  # Less than 10% should be unknown structure errors
