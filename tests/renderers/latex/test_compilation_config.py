...
class TestCompilationConfig:
    ...
    def test_get_engine_command_draft_mode(self):
        """Test engine command generation in draft mode."""
        config = CompilationConfig(mode=CompilationMode.DRAFT)

        cmd = config.get_engine_command(LaTeXEngine.LUALATEX)
        assert "-draftmode" in cmd

    def test_should_run_multipass(self):
        """Test multipass compilation detection."""
        # Normal mode should run multipass
        config = CompilationConfig(mode=CompilationMode.NORMAL)
        assert config.should_run_multipass() is True

        # Draft mode should not run multipass
        config = CompilationConfig(mode=CompilationMode.DRAFT)
        assert config.should_run_multipass() is False

        # Single pass should not run multipass
        config = CompilationConfig(max_passes=1)
        assert config.should_run_multipass() is False

    def test_get_timeout_for_pass(self):
        """Test timeout calculation for passes."""
        config = CompilationConfig(timeout_seconds=300)
        ...
        config = CompilationConfig(timeout_seconds=100)
        assert config.get_timeout_for_pass(1) == 100
        assert config.get_timeout_for_pass(2) == 50  # min(50, 60) = 50

        # With very small timeout, still gets half
        config = CompilationConfig(timeout_seconds=60)
        assert config.get_timeout_for_pass(1) == 60
        assert config.get_timeout_for_pass(2) == 30  # min(30, 60) = 30

    def test_for_mode_draft(self):
    ...
    def test_for_mode_with_kwargs(self):
    ...
    def test_validate_max_passes_too_high(self):
        """Test validation of excessive max_passes."""
        config = CompilationConfig(max_passes=15)
        errors = config.validate()
        assert len(errors) == 1
        assert "max_passes should not exceed 10" in errors[0]

    def test_validate_valid_output_dir(self):
        """Test validation with valid output directory."""
        config = CompilationConfig(output_dir=Path("/tmp"))
        errors = config.validate()
        assert len(errors) == 0
...
class TestCompilationResult:
    ...
    def test_str_representation_no_error_message(self):
