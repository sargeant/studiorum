...
To run the adventure, you need the fifth edition core rulebooks (\textit{Player's Handbook}, \textit{Dungeon Master's Guide}, and \textit{Monster Manual}).
...
When a creature's name appears in \textbf{bold} type, that's a visual cue pointing you to its stat block as a way of saying, "Hey, DM, you better get this creature's stat block ready. You're going to need it." Usually, you can find the stat block in the \textit{Monster Manual}; if the stat block appears elsewhere, the text tells you so.

Spells and equipment mentioned in the adventure are described in the \textit{Player's Handbook}. Magic items are described in the \textit{Dungeon Master's Guide} unless the adventure's text directs you to an item's description in appendix B of this adventure.
...
If you want to run this adventure as part of a campaign that starts at 1st level, the adventure "\textit{Unwelcome Spirits}" in \textit{Explorer's Guide to Wildemount} is ideal for a group of 1st-level characters in Xhorhas and can be used to advance the characters to 3rd level.
...
10th & Enable the \textit{Jewel of Three Prayers} to transform into its Jewel of Three Prayers (Exalted) (see area M9).\\
...
During this adventure, the characters will explore sunken grottoes, submerged ruins, and an extraplanar aquatic abyss. To run these underwater scenarios in a smooth and fun manner, familiarize yourself with the \textit{underwater combat rules} in the \textit{Player's Handbook} as well as the rules below.
...
A creature with a swimming speed—including a character with a \textit{ring of swimming} or similar magic—can swim all day without penalty and uses the normal forced march rules in the \textit{Player's Handbook}.
...
Only a \textit{wish} spell or divine intervention can cure a creature's ruidium corruption; simply removing all levels of exhaustion from the creature cannot.
...
When the characters meet the rivals in chapter 1, all the rivals have an indifferent attitude toward them. It's up to you to determine when a rival's attitude toward a character might change. For example, after a combat encounter in which one character saves a rival's life or helps a rival achieve one of their goals (see the Rivals' Goals table), you can decide if that deed enabled the rival to see the character in a new light, changing the rival's attitude toward the character from hostile to indifferent or indifferent to friendly. The \textit{rules for changing attitude} are detailed in the \textit{Dungeon Master's Guide}.
...
At times, the characters might want help from one or more rivals to accomplish their goals. Any character who requests help from a rival must make a successful Charisma check to convince that rival to help them. Consider that rival's relationship to the character in question, then consult the \textit{Conversation Reaction} table in the \textit{Dungeon Master's Guide} to determine the DC of the check and the rival's reaction to the character's request.

\subsubsection{Coming to Blows}
As long as the characters haven't slain any of them, the rivals do their best to avoid killing the characters. If a battle erupts between the two groups, the rivals try to knock the characters unconscious (see the \textit{rules} for doing so in the \textit{Player's Handbook}). In situations where that's not possible, Dermot Wurder uses his \textit{spare the dying} spell to stabilize a dying character.
...
Before starting the adventure, consider spending a game session—colloquially called session zero—to establish expectations, share house rules, and help your players create characters. If you have a copy of \textit{Tasha's Cauldron of Everything}, see that book's "\textit{Session Zero}" section for advice on how to lay the groundwork in session zero for a fun campaign that exceeds your players' expectations.

All the character races presented in the \textit{Player's Handbook} are well suited for this adventure. If your players have access to \textit{Explorer's Guide to Wildemount}, the race and subclass options presented in that book are available to them as well. With your consent, a character can also receive the \textit{Hollow One} supernatural gift described in that book.

Additional race options appear in \textit{Monsters of the Multiverse}, with bugbear, duergar, goblin, hobgoblin, kobold, lizardfolk, minotaur, and orc being appropriate choices for players who want their characters to come from cultures or societies with roots in Xhorhas.

All class options presented in the \textit{Player's Handbook} can be used in this adventure, and players can choose options from other D\&D books with your approval.
...
\textbf{Aradrine the Owl} & AWR-uh-dreen & Goliath coleader of the Consortium of the Vermilion Dream\\
...
A character can try to change a rival's attitude through roleplaying, which you can adjudicate based on the \textit{social interaction rules} in the \textit{Dungeon Master's Guide}. You can also improvise and adjust the rivals' attitudes at your discretion, based on how you feel the rivals would react in any given situation.
...
\subparagraph{Putting Away the Pies}
...
\subparagraph{Rival Impressions}
...
Each winner of this contest receives a tin \textit{medal of the meat pie} (see appendix B) shaped like—you guessed it—a meat pie.
...
\subparagraph{Studying the Maze}
...
\subparagraph{Entering the Maze}
...
\subparagraph{Meeting Maggie and Dermot}
...
\subparagraph{Treasure}
Characters who complete the maze are awarded a \textit{medal of the maze} (see appendix B) made of smooth driftwood, hung on a piece of rough twine. A labyrinthine pattern has been carved into the front of the medal.
...
The blue-skinned individual is a water genasi named \textbf{Ayo Jabe (tier 1)}, whose stat block appears in appendix A. After breaking up the argument on the docks, she returns to her companion, an elegant drow in wizard's robes. This is \textbf{Galsariad Ardyth (tier 1)}, whose stat block also appears in appendix A.
...
\subparagraph{Participating in the Challenge}
...
\subparagraph{Race for the Spear}
...
\subparagraph{Danger from Below}
...
\subparagraph{Meeting Ayo and Galsariad}
...
\subparagraph{Treasure}
Omo awards the winner a \textit{medal of the conch} (see appendix B) made of sea glass that has been magically crafted into the shape of a conch shell.
...
\subparagraph{Treasure}
Vars hands anyone who bests Maryl and Maggie a \textit{medal of muscle} (see appendix B) made of ebony and shaped like a flexing arm. If one stares at it for long enough, the muscles appear to bulge.
...
\subparagraph{Harvesting the Rice}
...
\subparagraph{Orc Team}
Athletics +3, Sleight of Hand +0

\subparagraph{Drow Team}
...
\subparagraph{Treasure}
Beetle and Zag award each member of the winning team a copper \textit{medal of the wetlands} (see appendix B) engraved with a sheaf of rice, hung on a necklace woven of reeds.
...
\subparagraph{Maneuvering the Herd}
...
If a tortoise is charmed by an \textit{animal friendship} spell or a similar effect, Wisdom (Animal Handling) checks to guide it are made with advantage, and it won't buck its handler on a failed check.

\subparagraph{Treasure}
Once a tortoise reaches its destination, Adan awards its rider a \textit{medal of the horizonback} (see appendix B).
...
\subparagraph{Table 1: The Box's Riddle}
...
\subparagraph{Table 2: The Star Map's Riddle}
...
\subparagraph{Table 3: The Bottles' Riddle}
...
\subparagraph{Meeting Galsariad}
...
\subparagraph{Treasure}
If a character answers at least two of the riddles correctly, Elder Colbu Kaz awards that character a \textit{medal of wit} (see appendix B), which is sculpted and painted to look like the head of a fox.
...
This is the first fully submerged chamber of the grotto. The characters must make a DC 13 Strength (Athletics) group check as they try to swim through the mass of kelp (see the \textit{Player's Handbook} for the rules on \textit{group checks}). The result of this check determines the time it takes them to move through the chamber.
...
\subparagraph{Moonshark's Defeat}
...
The object on the crystal pedestal glitters like the sunrise. This is the \textit{Jewel of Three Prayers}, a Vestige of Divergence that will drive the course of the adventure from now on. The Jewel of Three Prayers (Dormant) is currently in its Dormant State (see appendix B for the description of this magic item).
...
\item[Hostile Rivals.]
The characters awaken to discover that the rivals have absconded with both the Emerald Eye and the \textit{Jewel of Three Prayers}.
\end{description}

\subparagraph{Disputing Destiny}
...
\DndDropCapLine{I}{}n this chapter, the characters learn more about the events that occurred in the Emerald Grotto and the mythic significance of the \textit{Jewel of Three Prayers}. Following this discovery, the characters depart Jigow and travel across Xhorhas toward Bazzoxan, a bleak military outpost guarding a fortress of evil.
...
\begin{description}
\item[Heroic Quest.]
In the Emerald Grotto, the characters obtained the \textit{Jewel of Three Prayers} and experienced the vision from the Apotheon. In this scenario, they are approached by Elder Ushru, who senses that the \textit{Jewel of Three Prayers} is a powerful magic item and suggests the characters undertake a journey to learn more about it.
\item[Ours by Right.]
...
\begin{itemize}
\item Dermot is genuinely concerned for the characters' well-being. He mentions that he heard what sounded like a voice in the grotto and wants to know if the characters heard it too.
\item Galsariad is curious about what the characters learned in their vision and is eager for the characters to share any magical secrets they might have discovered.
\item Irvan has no interest in what happened in the grotto, but he is interested in learning what the jewel can do.
\item Maggie agrees with anyone who expresses concern for the characters but doesn't say much on her own. She has her suspicions that more happened than what they have related during the event in the grotto, but she won't push them if they don't want to reveal the full truth.
\end{itemize}
...
A character who notices the thief can try to capture them. Doing so requires a successful DC 15 Dexterity check to catch up to the thief followed by a successful grapple check (see the \textit{Player's Handbook}). If either check fails, the thief escapes.
...
How long the journey takes and the number of random encounters that occur depend on whether the characters are moving at a normal, fast, or slow pace (see the rules for \textit{travel pace} in the \textit{Player's Handbook}):
...
Just off the road ahead, the characters spot the corpse of a \textbf{gloomstalker} (see appendix A). Even in death, its corpse is covered in roiling, wispy shadows. The grass around the creature has been trampled flat.
...
\begin{description}
\item[Patrol at Rest.]
The characters come across six lawful neutral Aurora Watch veterans (a mix of lawful good drow, humans, and orcs) who are resting at a campsite just off the road. Clearly in good spirits, they invite the characters to join them. Traveling with this patrol is a lawful good, drow \textbf{scout} who also serves as a cook. If the characters take a long rest with the patrol, the food they were given the night before grants them 1d10 \textit{temporary hit points} when they awaken the next day.
\item[Patrol in Battle.]
The characters see six lawful neutral Aurora Watch veterans (a mix of lawful good drow, humans, and orcs) fighting a \textbf{hezrou}. If the characters help the Aurora Watch slay the demon, the soldiers thank them, give them a total of 50 gp as a reward, and promise to commend them to Taskhand \textbf{Verin Thelyss} when the patrol returns to Bazzoxan. If the characters do nothing to help, the soldiers slay the demon, but only after one soldier is killed and two others are reduced to 0 hit points (one stable, the other dying).
\item[Lost Patrol.]
On the road in front of the characters are the bodies of six Aurora Watch soldiers near the corpse of a hezrou that is slowly dissolving into black ichor. Two shadow demons and two quasits lurk beneath the soldiers' corpses, waiting for more prey to arrive. They fight to the death.
\end{description}
...
The carrion bird is a \textbf{vrock} that has been feeding on the mortal flesh strewn about the battlefield. It swoops down to attack characters who disturb any of the corpses. In addition, three quasits hide in the udaak's fur and attack non-demons that come within 10 feet of the udaak's melting corpse.
...
The characters see a \textbf{gloomstalker} (see appendix A) antagonizing 1d3 mammoths that are moving slowly across the wastes, minding their own business. The creatures are 3d6 × 100 feet away from the characters when the encounter begins.
...
Each time the gloomstalker uses Shriek during the battle against the characters, 1d6 skeletons of dead human soldiers burst up out of the ground within 30 feet of the characters and attack them, acting on their own initiative count. Skeletons that aren't reduced to 0 hit points collapse into inanimate piles of bones after 10 minutes or when the gloomstalker is slain.
...
The characters hear a wild yell. In the distance, they see a lawful neutral, tiefling \textbf{scout} named Justice on the back of a \textbf{moorbounder} (see appendix A) and hanging on for dear life as the creature snarls and bucks about. Two bristled moorbounders (see appendix A) are circling the imperiled tiefling. Justice can hang on for another minute at most, and the moment he touches the ground, the bristled moorbounders intend to tear him limb from limb.
...
\begin{description}
\item[Jewel Theft.]
If the characters intend to take the \textit{Jewel of Three Prayers} from the rivals, this will likely be a combat encounter or an attempted infiltration.
\item[No Quarrel.]
...
Big Yuyo is an adult \textbf{horizonback tortoise} (see appendix A)—known as a "kinespaji" in Goblin—that is the home of the Kruzkrenner goblin family. The tortoise is named after a Xhorhasian vegetable called a yuyo—an ochre cucumber that grows in Rosohna's underground gardens.
...
\subparagraph{Big Yuyo Locations}
...
\item[L2f:]
Command Post. The command post holds Drazagorr and Kelbadurn's bed and their belongings, which include a set of goblin-sized \textit{chain mail}, a \textit{spear}, a \textit{shield}, two Shortbow, two Quiver containing 20 Arrows (20) apiece, and a wooden chest that holds 100 gp (the family's savings) and a collection of sentimental keepsakes.
\end{description}
...
\item The Acorn Sisters, Lanata and Robur, are dryads. As the caretakers of this forest, they're fine with bonfires as long as only dead wood is burned. It's customary to share food, drink, and song with them in appreciation for their hospitality. Characters who show them great respect by making an offering directly to them awaken in the morning with a \textit{charm of vitality} (see the rules for \textit{supernatural gifts} in the \textit{Dungeon Master's Guide}).
\end{itemize}

\subparagraph{Shopping}
The caravanners parked around the bonfire—including Justice's employers, if he is present—have merchandise for sale. Two neutral bugbear chiefs guard the wagons, and six lawful neutral Aurora Watch veterans (a mix of drow, humans, and orcs) resting at the camp are prepared to apprehend any thieves and drag them to Bazzoxan in chains.
...
\begin{DndTable}[header=Merchants' Wares]{XXc}
...
\textit{Quiver} of 20 Arrows (20) & 2 gp & 1d4\\
\textit{Quiver} of 20 Crossbow Bolts (20) & 2 gp & 1d4\\
\textit{Sack} & 1 cp & 2d6\\
...
Five gibbering mouthers have sneaked into Bazzoxan and are intent on consuming anyone in their path. Opposing them are members of the Aurora Watch led by Taskhand \textbf{Verin Thelyss} (see the accompanying stat block).
...
\subsection{Fight!}
If the characters choose to help the Aurora Watch, everyone rolls initiative. Verin and two orc guards of the Aurora Watch stay with the party, while the rest of the soldiers leave to secure other parts of the city.
...
4 & An Aurora Watch soldier (lawful good \textbf{drow elite warrior}) breaks up a fight between two sobbing citizens (lawful good commoners).\\
5 & Ten Aurora Watch guards (lawful good drow, humans, and orcs) jog in formation, keeping tempo by singing about the ways they'll die.\\
...
The tiefling is \textbf{Prolix Yusaf}, a lawful neutral \textbf{scholarly agent} (see appendix A) from the Allegiance of Allsight. His badge displays the symbol of his faction: a scroll bearing a single, watchful eye.
...
\subparagraph{Roleplaying Prolix}
...
\subparagraph{Who is your rival}
...
\subparagraph{Do you know how to open the puzzle box or not}
...
\subparagraph{Prolix and the Jewel}
...
\subparagraph{Bautha and the Jewel}
...
\subparagraph{Sleeping Quarters}
...
\subparagraph{Foghome the Gardener}
...
\begin{DndSidebar}[float=!b]{Firbolgs of Exandria}
...
\subparagraph{Speech of Beast and Leaf}
...
\subparagraph{Foghome and the Jewel}
...
When the vision concludes, the characters fall prone on the ground, having momentarily passed out. \textbf{Foghome} doesn't see the vision, but he sees the characters fall unconscious and then awaken moments later. If they admit to seeing the vision, he asks them to describe it before nodding slowly and explaining he has seen the vision before. He believes the central figure in the vision to be a hero of the Calamity and recounts the old stories of Bazzoxan's origin:
...
\subparagraph{Question and Answers}
Question, a chaotic neutral, tiefling \textbf{monastic operative} (see appendix A), can be found lounging around the Ready Room at all hours. Unlike most others in Bazzoxan, Question almost always has a smile on her face.
...
\subparagraph{What Question Knows}
...
\subparagraph{Question and the Jewel}
...
\subparagraph{Question's Allies}
...
\subparagraph{Aloysia Telfan}
During their conversation with Question, characters who have a passive Wisdom (Perception) score of 12 or higher notice a gaunt figure in scarlet robes nearby, eavesdropping. This figure, \textbf{Aloysia Telfan}, is a lawful evil, elf \textbf{occult initiate} (see appendix A) who thirsts for knowledge and power. She approaches the characters, if they don't accost her first, and greets them politely.
...
\subparagraph{What Aloysia Knows}
...
\subparagraph{Roleplaying Aloysia}
...
\begin{description}
\item[Black Stone.]
Unless otherwise noted, all surfaces in the Betrayers' Rise are made of smooth, shiny black stone. The power that pervades the place prevents the architecture from being affected by magic such as \textit{stone shape}.
\item[Ceilings.]
...
If you want to or need to, you can create new maps for the Betrayers' Rise, new encounters there, and other routes through it. The Betrayers' Rise Encounters table provides some suggestions for unsettling creatures that a group might encounter while delving into these new areas, and you can use the \textit{treasure tables} in \textit{chapter 7} of the \textit{Dungeon Master's Guide} to generate treasure guarded by these creatures.

\begin{DndTable}[header=Betrayers' Rise Encounters]{cX}

d10 & Encounter\\
1 & The characters enter a room lined with flickering candles, only to discover that their shadows have detached from them and transformed into shadow demons (one per character).\\
2 & Four ochre jellies burble along the walls of a hallway, looking for prey to devour.\\
...
5 & Two invisible barlguras ambush the characters.\\
6 & A passageway opens into what was once a coliseum and now contains three enraged nightmares.\\
7 & A gurgling, acidic river cuts through a cavern where three ropers stealthily await.\\
8 & Three chasmes emerge from their hive, which has overtaken what was once a chandelier.\\
...
d6 & Encounter\\
1 & 1d6 quasits nesting in the corpses of the area's previous inhabitants or crawling on the walls in centipede form\\
2 & 1 \textbf{chasme} clinging to the ceiling\\
3 & 1 invisible \textbf{barlgura} standing guard\\
4 & 1d4 shadow demons lurking in the darkness\\
5 & 3d6 \textbf{manes} looking for a more powerful demon to lead them\\
6 & 1 \textbf{gloomstalker} (see appendix A) feeding on the corpse of a giant rat and hungry for more food\\
\end{DndTable}
...
\subparagraph{Secrets of the Skull}
...
\subparagraph{One-Way Secret Door}
...
\subparagraph{Opening the Double Door}
...
\subparagraph{Stained-Glass Mosaic}
...
\subparagraph{Chains of Darkness}
...
\subparagraph{Navigating the Labyrinth}
...
\subparagraph{Climbing the Chasm Walls}
...
\subparagraph{Bodies of the Fallen}
...
\subparagraph{Treasure}
A character who inspects the remains of the two Cobalt Soul explorers finds a \textit{cloak of protection} and a \textit{cracked driftglobe}. In its damaged state, the Cracked Driftglobe can be used only to cast the \textit{light} spell.
...
\subparagraph{Curse of the Crawling King}
...
\subparagraph{The Jewel Awakens}
...
The next time the characters look at the \textit{Jewel of Three Prayers}, they find it has transformed into its Jewel of Three Prayers (Awakened) (as described in appendix B). The transformed jewel can be removed from the altar safely.

\subsection{Confrontation for the Jewel}
The following boxed text assumes that \textbf{Aloysia Telfan} is still alive; if that's not the case, replace Aloysia with another \textbf{occult initiate} (see appendix A) from the Consortium of the Vermilion Dream:
...
Ayo Jabe congratulates the characters for reaching the prayer site and awakening the \textit{Jewel of Three Prayers}. The rivals not only allow the characters to keep the jewel but also offer to help them return to Bazzoxan. Aloysia is shocked by the rivals' betrayal.
...
Aloysia carries a satchel that holds three Teleportation Tablet (see appendix B), all connected to the same teleportation circle in Ank'Harel (see chapter 4). Her next actions are determined by who winds up with the \textit{Jewel of Three Prayers}.

\subparagraph{Aloysia Gets the Jewel}
...
\subparagraph{Aloysia Doesn't Get the Jewel}
...
\subparagraph{Dermot Wurder (Tier 2)}
...
\subparagraph{Galsariad Ardyth (Tier 2)}
...
\subparagraph{Irvan Wastewalker (Tier 2)}
...
\subparagraph{Maggie Keeneyes (Tier 2)}
...
Characters who use one of \textbf{Aloysia Telfan}'s Teleportation Tablet to travel to Ank'Harel arrive on a rooftop overlooking the Suncut Bazaar (see area T8 on the Streets of the Suncut Bazaar map). Read:
...
Aloysia leads the characters inside the storeroom, then asks them to wait while she goes to get someone. After a few minutes, she returns with one of the Consortium's leaders: Vrill the Moth, a lawful evil, elf \textbf{occult silvertongue} (see appendix A).
...
If Prolix used his \textit{teleportation tablet} to transport himself and the characters to Ank'Harel, the group arrives inside the Teleportation Atrium of the Crystal Chateau (area S2 on the Streets of the Sigil District map). If Question the tiefling is with them, she arrives here as well. Read:
...
The more charitable of the Allegiance of Allsight's two leaders is Alakritos, a neutral good, goblin \textbf{scholarly mastermind} (see appendix A). The headmaster is pleased to see that Prolix has returned from Bazzoxan safely and offers tea as he listens to Prolix and the characters talk.
...
\subparagraph{Allegiance Story Track}
...
\subparagraph{Consortium Story Track}
...
\subparagraph{Cobalt Soul Story Track}
...
\subparagraph{Inspecting the Oasis}
...
\subparagraph{Talking with Carliale}
...
When they see that the characters survived the surge, two invisible stalkers move in for the kill. The invisible stalkers were summoned by Professor Lymmle Wist and infiltrated the Life Dome a short time ago without Carliale's knowledge, slipping through the entrance as the gnome arrived for work. The invisible stalkers focus their attacks on the characters, ignoring Carliale.
...
\subparagraph{Interrogating the Excavators}
...
In gratitude, Prolix lets the characters keep the \textit{ring of red fury} (see appendix B) as long as they promise to share anything they learn about the ring's properties.
...
\subparagraph{Cliffside Villa (Chapter 5, Area M13)}
...
\subparagraph{Temple of the Arch Heart (Chapter 5, Area M9)}
This structure is Alyxian's final prayer site. If the characters haven't acquired the Key to the Netherdeep, advancing the \textit{Jewel of Three Prayers} to its Jewel of Three Prayers (Exalted) at this prayer site will enable them to enter the rift.

\subparagraph{Rift to the Netherdeep (Chapter 5, Area M17)}
...
If the characters enter First Eclipse (see the "Suncut Bazaar" section earlier in the chapter), they are greeted by the bartender, Satzrak Runestrider, a neutral evil, blue dragonborn \textbf{occult extollant} (see appendix A). If they ask him about the Consortium, he gives each of them a drink on the house and tells them to keep quiet. When they finish their drinks, Satzrak hisses, "Go home and wait for a letter."
...
Inside the Consortium's hidden storeroom in the back of the tavern, the characters find \textbf{Aloysia Telfan}. If Aloysia died earlier in the adventure, replace her with another \textbf{occult initiate} (see appendix A), and modify the following text accordingly:
...
If any of the thieves are confronted before they can pull off the heist, they try to flee the casino and foil any pursuers by running through the streets. Resolve such a chase using the \textit{chase rules} and the Urban Chase Complications table in the \textit{Dungeon Master's Guide}.
...
\subparagraph{Thieves Are Caught}
...
\item Rendezvous with \textbf{Galeokaerda}, who says she will be waiting inside Cael Morrow's Royal Library (area M7a in chapter 5).
...
\subparagraph{Royal Library Hall (Chapter 5, Area M7a)}
...
\subparagraph{Kelp Forest (Chapter 5, Area M11)}
...
\begin{itemize}
\item To open Cael Morrow's rift to the Netherdeep, one needs a \textit{ruidium weapon} (see appendix B). The Consortium has determined that any such weapon functions as a Key to the Netherdeep.
...
\subsubsection{Getting the Mission}
If the characters arrived in Ank'Harel with Question, the tiefling scholar they met in Bazzoxan in chapter 3, she greets them at their lodgings the morning after their arrival. Question knocks on the characters' door and cheerfully states that her superior, Iwo Zalarre (a chaotic good, half-orc \textbf{monastic operative}; see appendix A), wants to speak with them. If the characters accept the meeting, Question takes them to the Temple of the Mentor, the headquarters of the Cobalt Soul in Ank'Harel (see "Temple of the Mentor" earlier in the chapter).
...
The lockbox holds the \textit{ruidium shortsword} that Kruuk plans to sell (see "A Deal Goes Down" below). If the characters have gained Kruuk's trust, he opens the lockbox to show them the sword, then explains its magical properties (see appendix B).

\subsubsection{A Deal Goes Down}
Kruuk plans to meet one hour after sunset with \textbf{Ashann}, a chaotic neutral, gnome \textbf{occult extollant} (see appendix A) working for the Consortium of the Vermilion Dream. \textbf{Ashann} carries a pouch containing five peridots (500 gp each).
...
\textbf{Jamil A'alithiya}, a neutral good, human \textbf{monastic high curator} (see appendix A), offers a reward of 1,000 gp for completing the mission.
...
This combat encounter takes place on the Streets of the Suncut Bazaar map. The crystalline creatures are two ruidium elephants (see their stat block earlier in the adventure). These elephants are running amok and must be destroyed before they cause any further damage.
...
\item Earn Insight's trust and learn how to access the Netherdeep.
...
The characters can meet Insight in the Allegiance base camp (area M3 in chapter 5). She tells them that her companions are searching for the writings of an ancient hero who tried to save Cael Morrow from destruction during the Calamity. Earlier expeditions found dozens of stone tablets from the period in question at the south end of the excavation site. Her team, however, no longer has the force of arms to fight the dangerous creatures that lurk in this region. If the characters can find more of these tablets and return them to her, she'll be grateful.

\subparagraph{Insight's Reward}
If the characters bring back the tablet from area M10 in chapter 5, Insight shares the following information:


\begin{itemize}
\item Insight reveals the location of the rift leading to the Netherdeep (area M17 in chapter 5) and informs the characters that weapons infused with ruidium can open this rift.
\item Insight believes there might be keys to the rift in Cael Morrow—left behind by agents of the Consortium of the Vermilion Dream.
\end{itemize}
...
\subparagraph{Unstable Building (Chapter 5, Area M10)}
This location contains the tablet that Insight seeks.
...
Ayo Jabe. Ayo's drive to prove herself a hero, and to prove herself superior to the characters, has brought her to this place. She is aware of the trauma that her quest has put her friends through. This feeling has led her to doubt her leadership abilities. Ayo is filled with cold fury toward anything that stands in her way, including her own insecurities.

Dermot Wurder. Dermot's desire to protect his friends is still at the core of his being. But dozens of battles and near-death experiences have turned his uncertainty into steely confidence.

Galsariad Ardyth. Galsariad's haughtiness has drained from his heart—as have all his other emotions. He has witnessed unspeakable horrors in the name of gaining greater magical power and has protected himself from being overcome by fear and self-loathing by suppressing all his feelings.

Irvan Wastewalker. Irvan's cavalier demeanor disappeared when he lost his arm in a battle with a slithering bloodfin in Cael Morrow. His faction allies fitted him with a stylish and functional arcane prosthetic, but this brush with death has made him short-tempered, cautious, and mistrustful. Irvan fears that any further mistake could cost him his life—permanently.

Maggie Keeneyes. A deep melancholy has taken root in Maggie's heart. She continues the mission out of love for Ayo, but she fears that her leader's determination might not be enough to achieve victory. Maggie doesn't trust the Allegiance of Allsight agents in Cael Morrow (even if the rivals have allied with that faction), believing them to be influenced by a malevolent force. She has shared her misgivings about this organization with Ayo on several occasions.
...
Lurking in the waters of Cael Morrow is an aboleth that believes it is the physical embodiment of the Apotheon. It uses the \textbf{Alyxian Aboleth} stat block in appendix A.
...
d12 & Encounter\\
1 & A \textbf{swarm of sorrowfish} (see appendix A) flocks around the detritus of a shark's corpse, ravenous for more prey.\\
2 & Three scuttling serpentmaws (see appendix A) search for food.\\
3 & A \textbf{giant octopus} wrestles with a locked chest. The chest contains 1,000 ep in coins minted long ago in Cael Morrow.\\
4 & Tentacles burst from the darkness as a \textbf{death embrace} (see appendix A) notices the characters.\\
5 & Two slithering bloodfins (see appendix A) streak through the water, eager for a meal.\\
...
Two scholarly agents (see appendix A) stand outside the door to area M3. One is a lawful neutral human named Kal, and the other is a lawful good halfling named Nadiya.
...
If Insight, \textbf{Scribble}, and \textbf{Xot} become hostile toward intruders, they fight to kill. On the first turn of combat, Insight uses a \textit{sending stone} to contact Headmaster Cryon in the Crystal Chateau and request aid. Twenty minutes later, ten scholarly agents and five scholarly excavators (see appendix A for both stat blocks) arrive by way of area M1 to secure the base camp.

\subparagraph{Insight Acuere}
The tiefling professor uses the \textbf{scholarly mastermind} stat block (see appendix A), with these changes:
...
Insight won't cooperate with the Consortium of the Vermilion Dream unless she is tricked or magically compelled into doing so. If the characters are members of the Allegiance of Allsight, Insight informs them that they have arrived earlier than she expected, but she's nonetheless happy to see them. If they're allied with the Cobalt Soul, she requires them to prove their usefulness before she reveals any information (see "Insight's Quest" in mission 4 of the Cobalt Soul Story Track in chapter 4).

Insight is the primary archivist for the Cael Morrow archaeological project and would put her life on the line to ensure its success. She is willing to share the results of her research with fellow Allegiance members. Insight can offer the following information in response to the characters' questions:
...
\item A fourth member of the expedition, Insight's friend \textbf{Galeokaerda}, a high elf professor of ancient literature, hasn't been seen in three days. Insight is concerned but hasn't yet considered raising a search party because \textbf{Galeokaerda} has a shield guardian to protect her.
\end{itemize}

\subparagraph{Scribble}
\textbf{Scribble} uses the \textbf{scholarly agent} stat block (see appendix A), with this change:
...
\subparagraph{Xot}
This goliath uses the \textbf{scholarly excavator} stat block (see appendix A), with these changes:
...
\subparagraph{Exploring the Building}
...
\subparagraph{Malfunctioning Keystone}
...
If the characters leave the keystone undisturbed, they can pass through the tower safely and continue onward. If any of them return to the tower a second time, the keystone makes a loud popping noise in their presence and cracks in half, bringing down the magical barriers. The shark, which is a \textbf{corrupted giant shark} (see appendix A), attacks characters who now find themselves underwater.
...
\subparagraph{Buying Goods from Olara}
...
\textbf{Olara} has three bottles of this delicious, pre-Divergence vintage in stock and asks 50 gp per bottle. An Ank'Hareli sommelier will pay up to 1,000 gp for a bottle of this rare vintage.
\item[Potions of Healing.]
\textbf{Olara} has five of Potion of Healing in stock and sells them for 50 gp each.
\item[Potions of Heroism.]
\textbf{Olara} has two Potion of Heroism in stock and sells them for 300 gp each.
\end{description}

\subparagraph{Clearing Olara's Mind}
...
A neutral evil, high elf \textbf{occult extollant} (see appendix A) named \textbf{Galeokaerda} has set up camp in this room along with Crescent, her \textbf{shield guardian}. \textbf{Galeokaerda} wears the Master's Amulet around her neck but hides it under her shirt.

Modify Galeokaerda's stat block as follows:
...
As described in the faction story tracks in chapter 4, \textbf{Galeokaerda} has long posed as a professor of the Allegiance of Allsight and has become Insight's friend. Now she regrets betraying her friend and secretly hopes that she can convince Insight to turn against the Allegiance of Allsight when \textbf{Galeokaerda} and the Consortium of the Vermilion Dream take over Cael Morrow.
...
\subparagraph{Speaking with the Ghosts}
...
When a character experiences the second vision, the Alyxian Aboleth loses its ability to poison the emotions of nearby creatures, and the \textit{Jewel of Three Prayers} (see appendix B) transforms into its Jewel of Three Prayers (Exalted)—both consequences of gaining the blessing of Corellon the Arch Heart.
...
Four scuttling serpentmaws (see appendix A) lurk amid the rubble and scuttle across the floor to attack characters who investigate the room. Characters who have a passive Wisdom (Perception) score of 18 or higher notice the serpentmaws and aren't Surprise when they attack.

\subparagraph{Clay Tablets}
Characters who spend 10 minutes searching the floor of the building find, among other clay tablets that describe ancient prayers and catechisms of the monks of the temple of Corellon, the particular tablet that Insight has been seeking (see "Insight's Quest" in mission 4 of the Cobalt Soul Story Track in chapter 4). This tablet is written in Celestial script. It reads:
...
A \textbf{death embrace} (see appendix A) has made its lair in the ruins of this once magnificent location and attacks anyone who comes within reach of its tentacles. A \textbf{swarm of sorrowfish} (see appendix A) lurks among the tendrils of the \textbf{death embrace}, gaining cover. When the \textbf{death embrace} grapple a creature, the swarm loses its cover and the sorrowfish emerge to feed on the grappled creature.

\subparagraph{Treasure}
Embedded in the \textbf{death embrace}'s bell is a \textit{ruidium greataxe} (see appendix B), the so-called Key to the Netherdeep that the Allegiance of Allsight and the Consortium of the Vermilion Dream are after (see chapter 4). The axe can be removed from the \textbf{death embrace} only after the creature is dead.
...
The tower is haunted by eight sword wraith warriors under the command of Beltreath, a lawful neutral \textbf{sword wraith commander}. See appendix A for these creatures' stat blocks.
...
\subparagraph{Roleplaying the Spirits}
...
If combat breaks out and Beltreath is reduced to 50 hit points or fewer, he surrenders and begs the characters not to destroy him, for Hadarai's sake. If the characters show mercy, Hadarai is grateful and offers to assist them in any way he can. For example, the ghost, as a priest of Corellon, can help the characters bypass the magic ward around the temple (area M9) or can warn the characters about the aboleth that haunts the city.
...
\subparagraph{Hidden Symbol and Trapdoor}
...
The burst of light from this trap attracts the slithering bloodfins in area M16 and possibly the \textbf{Alyxian Aboleth} (see appendix A for their stat blocks). If the aboleth is in the nearby kelp forest (area M11) when the trap is sprung, it sees the light and immediately moves to investigate it.
...
Unless the characters killed them in area M15, three slithering bloodfins (see appendix A) prowl this area in search of prey. The characters spot them—three streaks of crimson darting through the ruins. Unless the characters move away from the ruins immediately, the bloodfins swim toward the intruders and attack.
...
If the characters have yet to defeat the \textbf{Alyxian Aboleth} (see appendix A), it senses their intrusion into this area and arrives 1 minute later, attacking the characters in a rage.
...
\subparagraph{Into the Netherdeep}
...
\begin{DndTable}[header=Fragments of Suffering]{cc}
...
\begin{description}
\item[Bedroom.]
\textbf{Meri}, on the cusp of adulthood, was training to be a soldier. Any character who spends at least 1 minute searching the bedroom can make a DC 12 Intelligence (Investigation) check. On a successful check, the character finds the name "\textbf{Meri}" carved into the empty sheath of a shortsword.
\end{description}

\subparagraph{Ghosts of the Remembered}
When the characters learn all three names, three ghosts rise from the women's corpses and attack the intruders until at least one of the ghosts' names is spoken aloud. The ghosts resemble the three women, but they are not restless spirits. Rather, they are manifestations of Alyxian's rage and regret. The characters can destroy these ghosts by reducing each of them to 0 hit points.
...
After the second wave is defeated, the characters have 1 round to heal and prepare for the final wave: a \textbf{sword wraith commander} and four sword wraith warriors on the back of a \textbf{horizonback tortoise} (see appendix A for these creatures' stat blocks). Alyxian shares his thoughts as the tortoise lumbers toward the party:
...
If one or more characters tried to convince \textbf{Perigee} of the party's good intentions, their attempt to sway \textbf{Perigee} has a positive effect on the final battle against Alyxian (see "Emotional Healing" in chapter 7), regardless of the success of the attempt.
...
Sehanine might reach out to bards, rogues, druids, clerics, and the mischievous. Her childlike form appears to such individuals in dreams. If the characters need her aid, she might send the \textbf{deva} \textbf{Perigee} to offer them support or a place of refuge.
\end{description}
...
If you are unfamiliar with the monster stat block format, read the \textit{introduction} of the \textit{Monster Manual} before proceeding further. That book explains stat block terminology and gives rules for various monster traits—information that isn't repeated here.
...
\begin{DndTable}[header=Battle Chatter]{XX}
...
If instead you want to weaken the rivals to make them less of a physical threat or reinforce the danger of ruidium corruption, you can give one or more of the rivals 1d4 levels of exhaustion each. In this case, you'll need to keep track of each rival's exhaustion level and the condition's corresponding effects (see "Exhaustion" in the \textit{Player's Handbook}).

\subparagraph{Useful Magic}
...
\subparagraph{Irvan's Prosthetic Arm}
...
The magic items in this section are presented in alphabetical order. For the rules on \textit{magic items}, see the \textit{Dungeon Master's Guide}.

\begin{DndSidebar}[float=!b]{Vestiges of Divergence}
...
The cards in appendix D are meant to be given to players whose characters gain Fragments of Suffering in chapter 6 (see that chapter for more information).
...
\chapter{Fragments of Suffering}
...
\begin{DndMonster}[float=t]{Aboleth Spawn}
...
\begin{DndMonster}[float=t]{Agathe Silverspoon}
...
\begin{DndMonster}[float=t]{Aloysia Telfan}
\DndMonsterType{Small or Medium humanoid (sorcerer), any alignment}
\DndMonsterBasics[
armor-class = {12},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{light}, \textit{mage hand}}
  \DndInnateSpellLevel[1e]{\textit{detect magic}, \textit{levitate}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
...
\DndMonsterAction{Arcane Blast (Recharge 5\textendash 6)}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{guidance}, \textit{light}}
  \DndInnateSpellLevel[1e]{\textit{lesser restoration}, \textit{remove curse}, \textit{water breathing}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float*=b,width=\textwidth + 8pt]{Ancient Brass Dragon}
\begin{multicols}{2}
...
\begin{DndMonster}[float=t]{Animated Glass Statue}
...
\begin{DndMonster}[float=t]{Aradrine The Owl}
\DndMonsterType{Small or Medium humanoid (sorcerer), lawful evil}
\DndMonsterBasics[
armor-class = {13},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel[1]{\textit{dimension door}}
  \DndInnateSpellLevel[2e]{\textit{charm person}, \textit{mage armor}, \textit{sending}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
...
\begin{DndMonster}[float=t]{Ashann}
\DndMonsterType{Small or Medium humanoid (sorcerer), any alignment}
\DndMonsterBasics[
armor-class = {12},
hit-points = {\DndDice{12d8 + 12}},
...
\begin{DndMonster}[float=t]{Ayo Jabe (Tier 1)}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel[1e]{\textit{animal friendship}, \textit{goodberry}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Ayo Jabe (Tier 2)}
\DndMonsterType{Medium humanoid (ranger, ), chaotic good}
\DndMonsterBasics[
armor-class = {15 (studded leather armor)},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel[1e]{\textit{animal friendship}, \textit{goodberry}, \textit{pass without trace}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel[1e]{\textit{animal friendship}, \textit{freedom of movement}, \textit{goodberry}, \textit{pass without trace}, \textit{protection from energy}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
Ayo makes three Harpoon or Longbow attacks.
...
\begin{DndMonster}[float=t]{Bugbear Chief}
\DndMonsterType{Medium humanoid (goblinoid), chaotic evil}
\DndMonsterBasics[
...
\begin{DndMonster}[float=t]{Celeste}
...
\begin{DndMonster}[float=t]{Dancing Flame}
...
\begin{DndMonster}[float=t]{Dermot Wurder (Tier 1)}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{guidance}, \textit{light}, \textit{spare the dying}}
  \DndInnateSpellLevel[1e]{\textit{bless}, \textit{cure wounds}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Dermot Wurder (Tier 2)}
\DndMonsterType{Small humanoid (cleric, goblin, goblinoid), lawful good}
\DndMonsterBasics[
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{guidance}, \textit{light}, \textit{spare the dying}}
  \DndInnateSpellLevel[1]{\textit{revivify}}
  \DndInnateSpellLevel[2e]{\textit{bless} (at 2nd level), \textit{cure wounds} (at 3rd level)}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
Dermot makes one Warhammer attack and one Searing Wrath attack.
...
\DndMonsterAction{Special Equipment}
Dermot wears \textit{mariner's armor} and wields a ruidium warhammer (see appendix B). If Dermot rolls a 1 on an attack roll made with the warhammer, he must succeed on a DC 20 Charisma saving throw or gain 1 level of exhaustion.
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{guidance}, \textit{light}, \textit{spare the dying}}
  \DndInnateSpellLevel[1]{\textit{revivify}}
  \DndInnateSpellLevel[2e]{\textit{bless} (at 3rd level), \textit{cure wounds} (at 3rd level)}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonsterSpells}
  ...
  \DndInnateSpellLevel[1e]{\textit{darkness}, \textit{faerie fire}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Fiendish Orc}
\DndMonsterType{Medium humanoid (orc), chaotic evil}
\DndMonsterBasics[
...
\begin{DndMonster}[float=t]{Foghome}
\DndMonsterType{Medium humanoid (any race), any alignment}
\DndMonsterBasics[
...
\begin{DndMonster}[float=t]{Galeokaerda}
\DndMonsterType{Small or Medium humanoid (elf, sorcerer), neutral evil}
\DndMonsterBasics[
armor-class = {12},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{minor illusion}}
  \DndInnateSpellLevel[1e]{\textit{mage armor}, \textit{major image}, \textit{phantasmal force}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Galsariad Ardyth (Tier 1)}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{mage armor}, \textit{mage hand}}
  \DndInnateSpellLevel[1]{\textit{longstrider}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
...
\begin{DndMonster}[float=t]{Galsariad Ardyth (Tier 2)}
\DndMonsterType{Medium humanoid (drow, elf, wizard), neutral}
\DndMonsterBasics[
armor-class = {12},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{mage armor}, \textit{mage hand}}
  \DndInnateSpellLevel[1e]{\textit{longstrider}, \textit{scrying} (as an action), \textit{slow}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Drain Potential}
...
\DndMonsterAction{Special Equipment}
Galsariad wears a \textit{ring of swimming} and carries a ruidium dagger (see appendix B). If he rolls a 1 on an attack roll made with the dagger, he must succeed on a DC 20 Charisma saving throw or gain 1 level of exhaustion.

\DndMonsterAction{Spellcasting}
Galsariad casts one of the following spells, using Intelligence as the spellcasting ability (spell save DC 16):
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{mage hand}}
  \DndInnateSpellLevel[1e]{\textit{longstrider}, \textit{scrying} (as an action), \textit{slow}, \textit{telekinesis}, \textit{water breathing}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Gryz Alakritos}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {12},
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{alter self}, \textit{light}, \textit{mage hand}}
  \DndInnateSpellLevel[1e]{\textit{comprehend languages}, \textit{mage armor}, \textit{zone of truth}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Insight Acuere}
...
\begin{DndMonster}[float=t]{Irvan Wastewalker (Tier 1)}
...
\begin{DndMonster}[float=t]{Irvan Wastewalker (Tier 2)}
\DndMonsterType{Medium humanoid (human), chaotic neutral}
\DndMonsterBasics[
armor-class = {15 (studded leather armor)},
...
\DndMonsterAction{Special Equipment}
Irvan wears \textit{gloves of swimming and climbing} and wields a ruidium shortsword (see appendix B). If Irvan rolls a 1 on an attack roll made with the shortsword, he must succeed on a DC 20 Charisma saving throw or gain 1 level of exhaustion.
...
\begin{DndMonster}[float=t]{James Cryon}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {12},
hit-points = {\DndDice{15d8 + 15}},
...
\begin{DndMonster}[float=t]{Jamil A'Alithiya}
\DndMonsterType{Small or Medium humanoid (monk, wizard), any alignment}
\DndMonsterBasics[
...
\begin{DndMonsterSpells}
  ...
  \DndInnateSpellLevel[2e]{\textit{detect thoughts}, \textit{identify} (as an action), \textit{scrying} (as an action)}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\DndMonsterAction{Brain Burn (Recharge 4\textendash 6)}
...
\begin{DndMonster}[float=t]{Koris}
\DndMonsterType{Medium humanoid (any race), any non-good alignment}
\DndMonsterBasics[
armor-class = {15 (studded leather armor)},
hit-points = {\DndDice{12d8 + 24}},
...
\begin{DndMonster}[float=t]{Laurin Ophidas}
...
\DndMonsterAction{Special Equipment}
Laurin wields a \textit{staff of the adder} and is attuned to it as though he were a cleric.
...
\begin{DndMonster}[float=t]{Maggie Keeneyes (Tier 1)}
...
\begin{DndMonster}[float=t]{Maggie Keeneyes (Tier 2)}
\DndMonsterType{Large giant (ogre), lawful neutral}
\DndMonsterBasics[
armor-class = {15 (\textit{chain shirt})},
...
\begin{DndMonster}[float=t]{Marisa}
\DndMonsterType{Medium undead, any alignment}
\DndMonsterBasics[
armor-class = {11},
hit-points = {\DndDice{10d8}},
...
\begin{DndMonster}[float=t]{Monastic High Curator}
\DndMonsterType{Small or Medium humanoid (monk, wizard), any alignment}
\DndMonsterBasics[
armor-class = {19 (Unarmored Defense)},
hit-points = {\DndDice{20d8 + 40}},
...
\begin{DndMonster}[float=t]{Monastic Infiltrator}
\DndMonsterType{Small or Medium humanoid (monk), any alignment}
\DndMonsterBasics[
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{disguise self}}
  \DndInnateSpellLevel[2e]{\textit{detect thoughts}, \textit{invisibility}}
  \DndInnateSpellLevel[1e]{\textit{dimension door}, \textit{zone of truth}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Monastic Operative}
\DndMonsterType{Small or Medium humanoid (monk), any alignment}
\DndMonsterBasics[
...
\begin{DndMonster}[float=t]{Moonshark}
\DndMonsterType{Huge beast, unaligned}
\DndMonsterBasics[
armor-class = {13 (natural armor)},
hit-points = {\DndDice{11d12 + 55}},
...
\begin{DndMonster}[float=t]{Occult Extollant}
\DndMonsterType{Small or Medium humanoid (sorcerer), any alignment}
\DndMonsterBasics[
armor-class = {12},
hit-points = {\DndDice{14d8 + 28}},
...
\begin{DndMonster}[float=t]{Occult Initiate}
\DndMonsterType{Small or Medium humanoid (sorcerer), any alignment}
\DndMonsterBasics[
armor-class = {12},
hit-points = {\DndDice{12d8 + 12}},
...
\begin{DndMonster}[float=t]{Occult Silvertongue}
\DndMonsterType{Small or Medium humanoid (sorcerer), lawful evil}
\DndMonsterBasics[
armor-class = {13},
hit-points = {\DndDice{17d8 + 51}},
...
\begin{DndMonster}[float=t]{Olara}
...
\begin{DndMonster}[float=t]{Prolix Yusaf}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {11},
...
\begin{DndMonsterSpells}
  ...
  \DndInnateSpellLevel[1e]{\textit{comprehend languages}, \textit{identify} (as an action), \textit{mage armor}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Arcane Shock}
...
\begin{DndMonster}[float=t]{Ruidium Elephant}
...
\begin{DndMonster}[float=t]{Scholarly Agent}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {11},
hit-points = {\DndDice{4d8 + 4}},
...
\begin{DndMonster}[float=t]{Scholarly Excavator}
\DndMonsterType{Small or Medium humanoid, any alignment}
\DndMonsterBasics[
...
\DndMonsterSection{Bonus Actions}
\DndMonsterAction{Telekinetic Toss (Recharge 5\textendash 6)}
...
\begin{DndMonster}[float=t]{Scholarly Mastermind}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {12},
hit-points = {\DndDice{15d8 + 15}},
...
\begin{DndMonster}[float=t]{Scribble}
\DndMonsterType{Small or Medium humanoid (wizard), any alignment}
\DndMonsterBasics[
armor-class = {11},
hit-points = {\DndDice{4d8 + 4}},
...
\begin{DndMonster}[float=t]{Shira}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{chill touch}}
  \DndInnateSpellLevel[1e]{\textit{blindness/deafness}, \textit{ray of sickness}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Club}
\textsl{Melee Weapon Attack:} 2 to hit, reach 5 ft., one target. \textsl{Hit:} 2 (1d4) bludgeoning damage.
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{detect evil and good}, \textit{invisibility} (self only)}
  \DndInnateSpellLevel[3e]{\textit{blade barrier}, \textit{dispel evil and good}, \textit{resurrection}}
  \DndInnateSpellLevel[1e]{\textit{commune}, \textit{control weather}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Sorrowfish}
...
\begin{DndMonster}[float=t]{Verin Thelyss}
...
\begin{DndMonsterSpells}
  \DndInnateSpellLevel{\textit{detect thoughts}, \textit{web}}
  \DndInnateSpellLevel[1]{\textit{dominate person}}
\end{DndMonsterSpells}
\DndMonsterSection{Actions}
\DndMonsterAction{Multiattack}
...
\begin{DndMonster}[float=t]{Young Horizonback Tortoise}
\DndMonsterType{Huge beast, unaligned}
\DndMonsterBasics[
armor-class = {15 (natural armor)},
hit-points = {\DndDice{8d12 + 16}},
