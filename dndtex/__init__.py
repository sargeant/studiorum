...
class DndTexError(Exception): pass
class DndTexUnhandled(DndTexError): pass
...
class Util:
    ...
    def load_extra_data(kind, name, source):
        ...
        logging.debug("searching for extra data from %s for %s (%s)", file_name, name, source)
        try:
    ...
    def alignment_string(data):
        ...
        if len(data) == 4:
            if "L" not in data and "NX" not in data: return "any chaotic alignment"
            if "E" not in data and "NY" not in data: return "any good alignment"
            if "G" not in data and "NY" not in data: return "any evil alignment"
            if "C" not in data and "NX" not in data: return "any lawful alignment"

        if len(data) == 5:
            if "G" not in data: return "any non-good alignment"
            if "E" not in data: return "any non-evil alignment"
            if "L" not in data: return "any non-lawful alignment"
            if "C" not in data: return "any non-chaotic alignment"
        ...
        for creature in json_data:
            if creature['name'].lower() == name.lower():
                if "_copy" in creature:
                    c = Util.findCreatureData(creature['_copy']['name'],  creature['_copy']['source'])
                    c['name'] = name.title()
    ...
    def findDeityData(name, pantheon, source):
        ...
        for deity in json_data:
            if deity['name'].lower() == name.lower() and deity['source'].lower() == source.lower() and deity['pantheon'] == pantheon:
    ...
    def remove_tag(tag_string):
        ...
        if not match: return tag_string
    ...
    def joinConjunct(source, multi_str, duo_str):
        ...
        if len(source) == 2:
            ...
            return ", or ".join([f'{multi_str} '.join(source[:-1]), source[-1]] if len(source) > 2 else source)
    ...
    def convertImage(src):
        ...
        if src.endswith('.webp'):
            ...
            if not os.path.exists(dest):
                ...
                convert_result = subprocess.run(convert_command, check=True, capture_output=True)
...
class Item:
    def __init__(self, data):
        ...
        if "ac" in data:
            ...
            dexterityMax = 2 if item_type == "MA" and not data.get("dexterityMax") else data.get("dexterityMax")
            isAddDex = data.get("dexterityMax") or item_type in ("HA", "S")

            prefix = "+" if item_type == "S" else ""
            suffix = f" + Dex{f' (max {dexterityMax})' if dexterityMax else ''}" if isAddDex else ""

            damageParts.append(f"AC {prefix}{data.get('ac')}{suffix}")

        if data.get("acSpecial"):
            damageParts.append(data['acSpecial'] if data.get('ac') else f"AC {data['acSpecial']}")
...
class Creature:
    def __init__(self, data):
        ...
        if "legendaryGroup" in data:
            self.wide = True
            groups = json.load(open('data/bestiary/legendarygroups.json', 'r', encoding='utf8'))
    ...
    def _getTokenImgPath(self, mediaDir='5eimages'):
        ...
        if "tokenUrl" in ent:
            return ent['tokenUrl']
        if "token" in ent:
            return Util.convertImage(f"{mediaDir}/{ent['token']['source']}/{self._nameToTokenName(ent['token']['name'])}.webp")
        if "tokenHref" in ent:
            raise(DndTexError("Don't know how to handle tokenHref"))

        return Util.convertImage(f"{mediaDir}/bestiary/tokens/{ent['source']}/{self._nameToTokenName(ent['name'])}.webp")
    ...
    def _strTag(self, tag):
        if isinstance(tag, str): return tag 
        return ''
    ...
    def _comma_list_string(self, seq, sep):
        return ' and '.join([f'{sep} '.join(seq[:-1]), seq[-1]] if len(seq) > 2 else seq)
    ...
    def renderCreature(self, renderer):
        ...
        if c.spellcasting:
            for spellcasting in c.spellcasting:
                ...
                cast_type = "DndInnateSpellLevel" if name == "Innate Spellcasting" else "DndMonsterSpellLevel"
                ...
                if "daily" in spellcasting:
                    for freq in spellcasting['daily'].keys():
                        number = freq[:-1]
                        spell_list = Util.remove_tag(", ".join(spellcasting['daily'][freq]))
                        lines.append(f"  \\DndInnateSpellLevel[{number}]{{{spell_list}}}")
                        
                if "spells" in spellcasting:                
                    for level in range(9):
                        level = str(level)
                        if level not in spellcasting['spells']: continue
                        spell_list = Util.remove_tag(", ".join(spellcasting['spells'][level]['spells']))
                        if level == "0":
                            lines.append(f"  \\{cast_type}{{{spell_list}}}")
                        else:
                            slots = ''
                            if "slots" in spellcasting['spells'][level]:
                                slots = '[' + str(spellcasting['spells'][level]['slots']) + ']'
                            lines.append(f"   \\{cast_type}[{level}]{slots}{{{spell_list}}}")
        ...
        if c.legendary:
            ...
            for la in c.legendary:
                ...
                lines.append(f"\\DndMonsterLegendaryAction{{{la['name']}}}{{{content}}}")
