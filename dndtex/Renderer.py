...
class Renderer():
    ...
    def init_document(self):
        ...
        self.lines.append(f"\\documentclass[10pt,a4paper,twoside,bg=print,twocolumn,openany,nodeprecatedcode]{{{document_type}}}")
    ...
    def render(self):
        ...
        if self.addCreatureList:
            ...
            self.lines.append("\\setlist[description]{nolistsep,listparindent=3pt,topsep=6pt,font={\\bfseries\\sffamily\\small}}")
    ...
    def renderSection(self, depth, data):
        ...
        if data.get("name") and data.get("name").startswith("Appendix") and not self.inAppendix:
            # lines += ["\\onecolumn"]
            lines += ["\\appendix"]
        ...
        if "name" in data:
            if depth > len(titles) - 1:
                logging.info("Depth of renderSection exceeds titles: %d %s", len(titles), data['name'])
                depth = len(titles) - 1
    ...
    def renderTable(self, data):
        ...
        if "caption" in data: lines += [str(f"\\begin{{{table_type}}}[header=@CAPTION]".replace("@CAPTION", data.get("caption")) + alignmentStr + "\n")]
        else: lines += [f"\n\\begin{{{table_type}}}" + alignmentStr + "\n"]
        if (titles): lines += [str(" & ".join(titles) + "\\\\")]
        for row in data.get("rows"):
            if isinstance(row, list): row = ["".join(self.renderRecursive(1, cell)) for cell in row]
            elif isinstance(row, dict): row = self.renderRecursive(1, row)
    ...
    def renderInset(self, depth, data):
        lines = []
        name = ""
        if "name" in data: name = self.escapeTex(data.get("name"))
    ...
    def renderList(self, data, depth=0):
        ...
        if name:
            lines.append(f"{{\\par \\vspace {{ 9pt plus 3pt minus 3pt }} \\noindent  \\DndFontTableTitle{{{name}}} \\nopagebreak }}")
        ...
        for item in data.get("items"):
            if isinstance(item, str):
                ...
                elif item.get("type") == "inset": # What? Who puts an inset as a list item?!
    ...
    def renderImage(self, data, options=None):
        ...
        if image_type == "external":
            ...
            src_image_path = f"{self.processedImagePath}/downloaded/{os.path.basename(image_url)}"
    ...
    def renderStatblock(self, data):
        ...
        if not creature_data:
            logging.warning(f"Could not findCreatureData for {data['name']} ({data['source']})")
            return [data['name']]
    ...
    def getAlignmentsStr(alignments):
        alignKey = {"text-align-left":"X", "text-align-center":"c", "text-align-right": "r"}
        out = []
        if not alignments:
            return "{}"
