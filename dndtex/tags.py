...
class DndTag():
    wrap_command = None
    static_string = None
    def __init__(self, args, creatureList=None, spellList=None, renderer=None, itemList=None):
        self.args = args
...
class creatureList(DndTag):
    def get_content(self):
        ...
        for (name, source) in sorted(self.creatureList.items()):
            ...
            if not c._data:
                warnings.warn(f"Failed to locate creature: {name} ({source})", UserWarning)
            else:
                if c.token:
                    size = c.size
...
class sense     (DndTag): pass
class card      (DndTag): pass
class hazard    (DndTag): pass
class race      (DndTag): pass
class status    (DndTag): pass
class background (DndTag): pass

class adventure (DndTag): pass
class book      (italic): pass
