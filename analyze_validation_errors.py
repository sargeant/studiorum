                ...
                if isinstance(obj[key], list):
                    for i, item in enumerate(obj[key]):
                        if isinstance(item, dict):
                            results.append(
                                f"{path}.{key}[{i}]: dict in list field {item}"
                            )
                else:
...
def find_validation_patterns(obj: Any, path: str = "") -> Dict[str, List[str]]:
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            if key in ["name", "source", "author", "storyline"] and isinstance(
                value, (dict, list)
            ):
                patterns["type_mismatches"].append(
                    f"{new_path}: expected string, got {type(value).__name__} {value}"
                )

            # List fields that might contain single values
            elif key in ["entries", "headers", "authors"] and not isinstance(
                value, list
            ):
                patterns["type_mismatches"].append(
                    f"{new_path}: expected list, got {type(value).__name__} {value}"
                )
                ...
                try:
                    ...
                    if isinstance(obj, dict) and "choose" in obj:
                        ...
                        if isinstance(choose_value, list):
                            choice_types[
                                f"choose list of {len(choose_value)} items"
                            ].append(obj)
                        else:
                            choice_types[
                                f"choose {type(choose_value).__name__}"
                            ].append(obj)
                except:
        ...
        if result["success"]:
            ...
            if total_issues > 0:
                files_with_issues.append(
                    (result["file"], total_issues, result["patterns"])
                )
