        ...
        while tags:
            for tag_string in tags:
                if not isinstance(tag_string, str):
                    logging.error(
                        "Searching tag_string but it's a dict? %s", tag_string
                    )
                match = re.search(
                    r"\{(?P<type>[@#])(?P<name>.*?) (?P<args>.*?)\}", tag_string
                )
                if not match:
                ...
                if tag_name not in globals():
                    logging.warning(
                        "Tag %s is not implemented: %s(%s)",
                        match.group("type"),
                        tag_name,
                        tag_arguments,
                    )
                    tag_name = "unknown"
                tag = globals()[tag_name](
                    tag_arguments,
                    creatureList=InTextTagRenderer.creatureList,
                    spellList=InTextTagRenderer.spellList,
                    renderer=renderer,
