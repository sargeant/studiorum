...
def convert_adventure(
    ...
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output LaTeX file"),
    ...
    with_items: bool = typer.Option(True, "--items/--no-items", help="Include item lists"),
    with_creatures: bool = typer.Option(True, "--creatures/--no-creatures", help="Include creature lists"),
    compile_pdf: bool = typer.Option(False, "--pdf", help="Compile to PDF after conversion"),
):
    """
    ...
    async def _convert():
        try:
            ...
            if output_file is None:
                output_path = Path("output/adventures") / input_file.with_suffix('.tex').name
            else:
            ...
            with Progress() as progress:
                render_task = progress.add_task("[green]Rendering adventure...", total=None)
...
def convert_supplement(
    ...
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output LaTeX file"),
    title: Optional[str] = typer.Option(None, "--title", help="Document title"),
    content_types: List[str] = typer.Option(["all"], "--type", help="Content types to include"),
    with_images: bool = typer.Option(False, "--images", help="Include images"),
    compile_pdf: bool = typer.Option(False, "--pdf", help="Compile to PDF after conversion"),
):
    """
    ...
    async def _convert():
        try:
            ...
            # Determine output file
            if output_file is None:
                output_path = Path("output/supplements") / input_file.with_suffix('.tex').name
            else:
                output_path = output_file
            ...
            # Create render context
            context = RenderContext(
                title=title or f"Supplement: {input_file.stem.replace('-', ' ').title()}",
                include_images=with_images,
            ...
            renderer = LaTeXDocumentRenderer()
            with Progress() as progress:
                render_task = progress.add_task("[green]Rendering supplement...", total=None)
                result = renderer.render_document(content_items, context)
                progress.update(render_task, completed=100)
            ...
            rprint(f"[green]✓[/green] Supplement converted ({len(content_items)} items): {output_path}")
...
async def _compile_pdf(latex_path: Path):
    ...
    try:
        ...
        rprint(f"[red]LaTeX compilation failed:[/red]")
