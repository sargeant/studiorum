...
def add_source(
    ...
    source_type: str = typer.Option(..., "--type", "-t", help="Source type (github, directory)"),
    url: Optional[str] = typer.Option(None, "--url", "-u", help="URL for GitHub sources"),
    path: Optional[str] = typer.Option(None, "--path", "-p", help="Path for directory sources"),
    enabled: bool = typer.Option(True, "--enabled/--disabled", help="Enable or disable the source"),
    priority: int = typer.Option(1, "--priority", help="Priority order (lower = higher priority)"),
    auto_update: bool = typer.Option(True, "--auto-update/--no-auto-update", help="Enable automatic updates"),
):
    ...
    try:
        ...
        console.print(f"[red]Error:[/red] Invalid source type '{source_type}'. Valid types: github, directory")
        raise typer.Exit(1)
...
def remove_source(
    ...
    remove_data: bool = typer.Option(False, "--remove-data", help="Also remove cached data"),
):
    ...
    if remove_data and source.type == SourceType.GITHUB:
        ...
        try:
            ...
            console.print(f"[yellow]Warning:[/yellow] Failed to remove cached data: {e}")
...
@app.command("update")
def update_sources(
    name: Optional[str] = typer.Argument(None, help="Name of specific source to update"),
):
...
def source_info(
    ...
    info_text.append(f"[bold]Status:[/bold] {'Enabled' if info['enabled'] else 'Disabled'}")
    info_text.append(f"[bold]Priority:[/bold] {info['priority']}")
    info_text.append(f"[bold]Auto Update:[/bold] {'Yes' if info['auto_update'] else 'No'}")
