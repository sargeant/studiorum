...
def setup_wizard():
    ...
    if config.content_sources:
        ...
        if not Confirm.ask("\nDo you want to reconfigure sources?"):
            console.print("Setup cancelled. Use [bold]5e2pdf sources[/bold] to manage sources.")
            return
    ...
    console.print("1. [cyan]Use defaults[/cyan] - 5etools official + homebrew (recommended)")
...
def check_setup():
    ...
    console.print(f"[green]✅ {len(config.content_sources)} content sources configured[/green]")
    ...
    async def _check():
        try:
            ...
            console.print(f"[green]✅ {stats['total_files']} content files available[/green]")
            ...
            table.add_row("Content Sources", "✅ Configured", f"{len(config.content_sources)} sources")
            table.add_row("Content Files", "✅ Available", f"{stats['total_files']} files")
