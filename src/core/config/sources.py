...
class ContentSource(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        ...
        if not v.replace('-', '').replace('_', '').isalnum():
            raise ValueError("Source name must contain only alphanumeric characters, hyphens, and underscores")
        return v
...
class ContentConfiguration(BaseModel):
    ...
    content_sources: List[ContentSource] = Field(default_factory=list, description="List of content sources")
    
    # Cache and storage settings
    cache_dir: Path = Field(default_factory=lambda: Path.home() / ".cache" / "5e2pdf", description="Cache directory")
    config_dir: Path = Field(default_factory=lambda: Path.home() / ".config" / "5e2pdf", description="Config directory") 
    
    # Global update settings
    auto_update_enabled: bool = Field(default=True, description="Enable automatic updates")
    update_check_interval: str = Field(default="daily", description="How often to check for updates")
    
    # Content processing settings
    parallel_downloads: int = Field(default=3, description="Number of parallel downloads")
    content_index_ttl: int = Field(default=3600, description="Content index cache TTL in seconds")
...
class ContentConfigManager:
    ...
    def _get_default_config_path(self) -> Path:
        ...
        if os.name == 'nt':  # Windows
            config_dir = Path(os.environ.get('APPDATA', Path.home() / 'AppData' / 'Roaming')) / '5e2pdf'
        else:  # Unix-like (macOS, Linux)
            config_dir = Path(os.environ.get('XDG_CONFIG_HOME', Path.home() / '.config')) / '5e2pdf'
    ...
    def _create_default_config(self) -> ContentConfiguration:
        ...
        config.add_source(ContentSource(
