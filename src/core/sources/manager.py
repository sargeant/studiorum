...
from ..config.sources import ContentConfiguration, ContentSource, SourceType, get_content_config
...
class ContentSourceManager:
    ...
    async def ensure_all_sources(self) -> None:
        ...
        if github_sources and not self.github_manager.is_git_available():
            ...
            raise RuntimeError("Git is required for GitHub sources but is not installed")
    ...
    async def _ensure_directory_source(self, source: ContentSource) -> None:
        ...
        if not source.path or not source.path.exists():
            logger.error(f"Directory source '{source.name}' path does not exist: {source.path}")
            raise FileNotFoundError(f"Directory source path not found: {source.path}")
        
        if not source.path.is_dir():
            logger.error(f"Directory source '{source.name}' path is not a directory: {source.path}")
    ...
    async def build_content_index(self, force_rebuild: bool = False) -> None:
        ...
        logger.info(f"Content index built with {total_files} total files from {len(enabled_sources)} sources")
    ...
    def get_all_content_files(self) -> Dict[str, List[Path]]:
        ...
        if not self._index_built:
            raise RuntimeError("Content index not built. Call build_content_index() first.")
