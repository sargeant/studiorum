...
from src.core.models.content import ContentType, BaseContent
...
class LaTeXSpellRenderer(LaTeXContentRenderer):
    ...
    def render_content(self, content: BaseContent, context: RenderContext) -> str:
        ...
        variables = {
            ...
            "higher_levels": self._format_higher_levels(content, context) if content.higher_level else None
        }
    ...
    def _format_higher_levels(self, spell: Spell, context: RenderContext) -> Optional[str]:
        ...
        for entry in spell.higher_level:
            if isinstance(entry, str):
                ...
                for sub_entry in entry["entries"]:
                    higher_entries.append(self.process_text_with_tags(sub_entry, context))
...
class LaTeXCreatureRenderer(LaTeXContentRenderer):
    ...
    def render_content(self, content: BaseContent, context: RenderContext) -> str:
        ...
        variables = {
            ...
            "damage_resistances": self._format_damage_list(getattr(content, 'resist', None)),
            "damage_immunities": self._format_damage_list(getattr(content, 'immune', None)),
            "condition_immunities": self._format_condition_list(getattr(content, 'conditionImmune', None)),
    ...
    def _format_condition_list(self, condition_data: Optional[List[str]]) -> Optional[str]:
    ...
    def _format_traits(self, traits_data: Optional[List[Dict[str, Any]]], 
                      context: RenderContext) -> Optional[List[Dict[str, str]]]:
        ...
        for trait in traits_data:
            if isinstance(trait, dict):
                ...
                for entry in entries:
                    if isinstance(entry, str):
                        description_parts.append(self.process_text_with_tags(entry, context))
    ...
    def _format_actions(self, actions_data: Optional[List[Dict[str, Any]]],
                       context: RenderContext) -> Optional[List[Dict[str, str]]]:
...
class LaTeXItemRenderer(LaTeXContentRenderer):
    ...
    def render_content(self, content: BaseContent, context: RenderContext) -> str:
        ...
        variables = {
            ...
            "description": self._format_entries(getattr(content, 'entries', []), context),
