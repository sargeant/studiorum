...
def extract_spell_references_legacy(text: str) -> list[dict[str, Any]]:
    """Extract spell references in legacy format for backward compatibility.

    Args:
        text: Text to parse

    Returns:
        List of spell reference dictionaries in legacy format
    """
    parser = SpellReferenceParser()
    ...
    for ref in references:
        legacy_ref = {
            "name": ref.target,
            "source": ref.source_book,
            "display_text": ref.display_text,
            "original_tag": ref.original_text,
        }
        legacy_refs.append(legacy_ref)

    return legacy_refs


def resolve_spell_references_legacy(
    references: list[dict[str, Any]], omnidexer: Omnidexer
) -> list[BaseContent]:
    """Resolve spell references in legacy format.

    Args:
        references: Legacy spell reference dictionaries
        omnidexer: Omnidexer for content lookup

    Returns:
        List of resolved BaseContent objects
    """
    resolver = ContentReferenceResolver(omnidexer)
    resolved = []

    for ref_dict in references:
        # Convert legacy format to unified Reference
        ref = Reference(
            source=ref_dict.get("original_tag", ""),
            target=ref_dict["name"],
            content_type=BaseContent,
            source_book=ref_dict.get("source"),
            display_text=ref_dict.get("display_text"),
            reference_type=ReferenceType.CONTENT,
        )

        content = resolver.resolve(ref)
        if content:
