...
class ValidationReport:
    ...
    async def run_full_analysis(self):
        ...
        self.results = {
            "summary": {
                ...
                "total_files_processed": len([f for files in data_paths.values() for f in files if f.exists()]),
    ...
    async def _analyze_file_structure(self, data_paths: Dict[ContentType, List[Path]]) -> Dict:
        ...
        for content_type, files in data_paths.items():
            ...
            if existing_files:
                ...
                type_analysis["total_size_mb"] = sum(size for _, size in sizes) / (1024 * 1024)
    ...
    async def _test_individual_loaders(self, data_paths: Dict[ContentType, List[Path]]) -> Dict:
    ...
    def _generate_recommendations(self, log_analysis: Dict, load_stats: Dict) -> List[str]:
    ...
    def generate_report_text(self) -> str:
        ...
        if file_analysis["suspicious_files"]:
            ...
            for file_info in file_analysis["suspicious_files"][:5]:
                lines.append(f"{file_info['file']}: {file_info['reason']} "
