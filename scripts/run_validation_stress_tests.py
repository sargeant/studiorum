...
def main():
    ...
    if passed == total:
        ...
        cli_check_cmd = ["python", "-m", "src.cli.main", "list", "content", "--limit", "10"]
        print(f"Running CLI validation check: {' '.join(cli_check_cmd)}")
        
        try:
            result = subprocess.run(cli_check_cmd, capture_output=True, text=True, timeout=60)
            stderr_lines = result.stderr.split('\n') if result.stderr else []
            warning_lines = [line for line in stderr_lines if 'WARNING' in line and 'Validation failed' in line]
            
            if warning_lines:
                print(f"⚠️  Found {len(warning_lines)} validation warnings in CLI output")
